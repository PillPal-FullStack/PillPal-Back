...
public class MedicationController {
    ...
    public ResponseEntity<List<MedicationResponse>> searchMedicationsByUserIdAndName(
        ...
        List<MedicationResponse> responses = medicationService.searchMedicationsByUserIdAndName(userId, name.trim());
        return ResponseEntity.ok(responses);
    }

    @PutMapping("/{id}/taken")
    @Operation(summary = "Mark medication as taken", description = "Marks a medication as taken by ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Medication successfully marked as taken"),
            @ApiResponse(responseCode = "404", description = "Medication not found"),
            @ApiResponse(responseCode = "401", description = "User not authenticated"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public ResponseEntity<Map<String, Object>> markAsTaken(
            @Parameter(description = "Medication ID", required = true) @PathVariable Long id) {

        log.info("Received request to mark medication {} as taken", id);

        try {
            medicationIntakeService.markAsTaken(id);
            ...
            log.error("Error marking medication {} as taken: {}", id, e.getMessage());
            throw e;
        }
    }

    @PutMapping("/{id}/skipped")
    @Operation(summary = "Mark medication as skipped", description = "Marks a medication as skipped by ID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Medication successfully marked as skipped"),
            @ApiResponse(responseCode = "404", description = "Medication not found"),
            @ApiResponse(responseCode = "401", description = "User not authenticated"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public ResponseEntity<Map<String, Object>> markAsSkipped(
            @Parameter(description = "Medication ID", required = true) @PathVariable Long id) {

        log.info("Received request to mark medication {} as skipped", id);

        try {
            medicationIntakeService.markAsSkipped(id);

            Map<String, Object> response = new HashMap<>();
            response.put("success", true);
            response.put("message", "Medication marked as skipped");
            response.put("medicationId", id);

            return ResponseEntity.ok(response);
        } catch (Exception e) {
            log.error("Error marking medication {} as skipped: {}", id, e.getMessage());
            throw e;
        }
    }

    @GetMapping("/{id}/intakes")
    @Operation(summary = "Get medication intake history", description = "Returns intake history for a specific medication")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Intake history obtained", content = @Content(schema = @Schema(implementation = MedicationIntake.class))),
            @ApiResponse(responseCode = "404", description = "Medication not found"),
            @ApiResponse(responseCode = "500", description = "Internal server error")
    })
    public ResponseEntity<List<MedicationIntake>> getMedicationIntakes(
            @Parameter(description = "Medication ID", required = true) @PathVariable Long id) {

        log.info("Received request to get intake history for medication: {}", id);
        List<MedicationIntake> intakes = medicationIntakeService.getIntakesByMedicationId(id);
        return ResponseEntity.ok(intakes);
    }

    @GetMapping("/{id}/intakes/date-range")
    @Operation(summary = "Get medication intake history by date range", description = "Returns intake history for a specific medication within date range")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Intake history obtained", content = @Content(schema = @Schema(implementation = MedicationIntake.class))),
            @ApiResponse(responseCode = "404", description = "Medication not found"),
    ...
    public ResponseEntity<List<MedicationIntake>> getMedicationIntakesByDateRange(
            @Parameter(description = "Medication ID", required = true) @PathVariable Long id,
        ...
        log.info("Received request to get intake history for medication {} between {} and {}", id, startDate, endDate);

        try {
            java.time.LocalDateTime start = java.time.LocalDateTime.parse(startDate);
            java.time.LocalDateTime end = java.time.LocalDateTime.parse(endDate);

            List<MedicationIntake> intakes = medicationIntakeService.getIntakesByMedicationIdAndDateRange(id, start,
                    end);
            return ResponseEntity.ok(intakes);
        } catch (Exception e) {
